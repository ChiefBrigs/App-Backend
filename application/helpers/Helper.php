...
class Helper
{
    ...
    /**
     * Function to uploads new  backups
     * @param $array
     * @param string $dir
     * @return null|string
     */
    public function uploadBackup($array, $dir = './')
    {
        if (!empty($array)) {
            $tmp = $array["tmp_name"];
            $name = $array["name"];
            ...
            if (is_dir($dir . 'uploads/backupFiles/' . $day_folder)) {
                $path = $day_folder;
            } else {
                if (mkdir($dir . 'uploads/backupFiles/' . $day_folder)) {
                    $path = $day_folder;
                } else {
                    $path = '';
                }
            }
            if (move_uploaded_file($tmp, $dir . 'uploads/backupFiles/' . $path . '/' . $new_name)) {
                $backupHash = md5($tmp . $name . uniqid() . time());
                $backupData = array(
                    'backup_original_name' => $this->_DB->escapeString($name),
                    'backup_new_name' => $new_name,
                    'backup_path' => $path,
                    'backup_hash' => $backupHash
                );
                $query = $this->_DB->insert('backups', $backupData);
                if ($query) {
                    return $backupHash;
                } else {
                    return null;
    ...
    /**
     * function to get backup file url
     * @param $Hash
     * @return null|string
     */
    public function getBackupFileUrl($Hash)
    {
        $Hash = $this->_DB->escapeString($Hash);
        $query = $this->_DB->select('backups', '*', "`backup_hash` = '$Hash'");
        if ($this->_DB->numRows($query) != 0) {
            $fetch = $this->_DB->fetchAssoc($query);
            $path = 'uploads/backupFiles/' . $fetch['backup_path'] . '/' . $fetch['backup_new_name'];
            return $path;
        } else {
            return null;
        }
